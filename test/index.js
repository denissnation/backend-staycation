...
describe('API Landing Page', () => {
	...
	it('POST Booking Page', (done) => {
		const imageUrl = __dirname + '/buktibayar.PNG';
		const dataSample = {
			imageUrl,
			idItem: '5e96cbe292b97300fc902223',
			duration: 2,
			bookingStartDate: '4/21/2020',
			bookingEndDate: '4/23/2020',
			firstName: 'Sahaja',
			lastName: 'Rons',
			emailAddress: 'sahaja@mail.com',
			phoneNumber: '08678686',
			accountHolder: 'Sahaja',
			bankFrom: 'BNI'
		};
		...
		chai
			...
			.attach('imageUrl', fs.readFileSync(dataSample.imageUrl), 'buktibayar.PNG')
			.end((err, res) => {
				expect(err).to.be.null;
